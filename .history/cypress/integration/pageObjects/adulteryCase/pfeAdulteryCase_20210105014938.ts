...
export class pfeAdulteryCase {
  ...
  basicAdulteryCase() {
    ...
    cy.get('.govuk-body-reference-number')
      ...
      const ccdCase=$caseid.text().trim()
      const ccdCaseId =ccdCase.replace(' - ',"")
      const ccdCaseId1 = ccdCaseId.split(' - ').join('')
      expect(ccdCaseId.split(' - ').join('')).eq(ccdCaseId.split(' - ').join(''))
      cy.log('Case ID -->'+ ccdCaseId.split(' - ').join(''))
  })
}}
